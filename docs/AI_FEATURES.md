# AI 助手功能开发指南

> 基于 Nuxt 3 + Supabase + 阿里云百炼的全栈 RAG AI 客服系统

## 目录

- [核心架构](#核心架构)
- [技术栈](#技术栈)
- [数据库设计](#数据库设计)
- [自动化向量化](#自动化向量化-edge-function)
- [应用层实现](#应用层实现-nuxt-3)
- [踩坑实录](#踩坑实录)
- [部署清单](#部署清单)

---

## 核心架构

整个 AI 系统分为两条数据流：

### 1. 自动化向量化 (ETL)

```
管理员更新产品 → Supabase Webhook → Edge Function → 生成向量 → 存入数据库
```

**目的**：保证 AI 始终基于最新的产品信息回答问题，无需人工干预。

### 2. 对话检索 (RAG)

```
用户提问 → 转向量 → 相似度搜索 → 组装 Prompt → LLM 生成 → 流式返回
```

---

## 技术栈

| 类别 | 技术选型 | 说明 |
|------|---------|------|
| **前端框架** | Nuxt 3 | Vue 3 SSR 框架 |
| **数据库** | Supabase | PostgreSQL + pgvector 扩展 |
| **大模型** | 阿里云通义千问 | Qwen-Plus (便宜、中文友好) |
| **向量模型** | 阿里云 text-embedding-v3 | **1024 维** (注意不是 1536) |
| **状态管理** | Pinia | 用于管理多会话状态、消息历史和持久化 |
| **AI SDK** | Vercel AI SDK v3 | `ai` + `@ai-sdk/openai` |
| **Markdown** | useSafeMarkdown | 基于 marked + DOMPurify 的安全渲染组件 |
| **匿名识别** | useAnonymousUser | 基于 FPJS 的浏览器指纹识别，管理 `anonymous_user_id` |
| **Token 管理** | checkUsageLimit | 服务端 Token 消耗估算与每日额度限制 (默认 10k) |
| **数据可视化** | ECharts | 用于管理后台 AI 统计图表展示 |

---

## 数据库设计

### 1. 启用向量扩展

```sql
-- 在 Supabase SQL Editor 中执行
create extension if not exists vector;
```

### 2. 创建向量表

**设计思路**：不直接修改原有的 `products` 表，而是创建关联表 `product_embeddings`。

```sql
create table public.product_embeddings (
  id bigint generated by default as identity primary key,
  product_id bigint references public.products(id) on delete cascade,
  content text, -- 清洗后的自然语言文本，给 AI 理解
  embedding vector(1024), -- 适配阿里云 text-embedding-v3 (1024维)
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);

-- 创建索引加速搜索
create index idx_product_embeddings_embedding_idx
  on public.product_embeddings
  using ivf (embedding vector_cosine_ops)
  with (lists = 100);
```

### 3. 创建向量搜索函数 (RPC)

这个函数会被 Nuxt 后端调用来检索相关产品。

```sql
create or replace function match_products (
  query_embedding vector(1024),
  match_threshold float default 0.5,
  match_count int default 5
)
returns table (
  id bigint,
  content text,
  similarity float,
  product_details jsonb
)
language plpgsql
as $$
begin
  return query
  select
    products.id,
    product_embeddings.content,
    1 - (product_embeddings.embedding <=> query_embedding) as similarity,
    to_jsonb(products.*) as product_details
  from product_embeddings
  inner join products on products.id = product_embeddings.product_id
  where 1 - (product_embeddings.embedding <=> query_embedding) > match_threshold
    and products.status = 'published'
  order by product_embeddings.embedding <=> query_embedding
  limit match_count;
end;
$$;

-- 授权给 service_role
grant execute on function match_products to service_role;
```

---

## 自动化向量化 (Edge Function)

为了实现"更新产品自动同步向量"，我们使用 Supabase Edge Function。

### 功能说明

当 `products` 表发生 `INSERT` 或 `UPDATE` 时：
1. Webhook 自动触发 Edge Function
2. Function 清洗 JSON 数据为自然语言
3. 调用阿里云 API 生成向量
4. 写入 `product_embeddings` 表

### 代码实现

**文件路径**：`supabase/functions/embed-product/index.ts`

```typescript
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'
import { OpenAI } from 'https://esm.sh/openai@4.42.0'

serve(async (req) => {
  try {
    const { record, type } = await req.json()

    // DELETE 操作跳过
    if (type === 'DELETE' || !record) {
      return new Response('Skipped', { status: 200 })
    }

    // 1. 数据清洗：JSON → 自然语言
    const name = record.name?.cn || record.name?.hk || record.name?.en || '未知产品'
    const specs = JSON.stringify(record.specs || {})
    const desc = record.description?.cn || record.description?.en || ''

    const content = `产品名称: ${name}\n规格参数: ${specs}\n产品描述: ${desc}`

    // 2. 调用阿里云生成向量
    const openai = new OpenAI({
      apiKey: Deno.env.get('DASHSCOPE_API_KEY'),
      baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
    })

    const embedding = await openai.embeddings.create({
      model: 'text-embedding-v3',
      input: content.replace(/\n/g, ' '),
      dimensions: 1024,
    })

    // 3. 写入数据库
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    )

    await supabase.from('product_embeddings').upsert({
      product_id: record.id,
      content: content,
      embedding: embedding.data[0].embedding,
    })

    return new Response(JSON.stringify({ success: true }), {
      headers: { 'Content-Type': 'application/json' },
    })

  } catch (error) {
    console.error('Embedding error:', error)
    return new Response(JSON.stringify({ error: error.message }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    })
  }
})
```

### 部署步骤

1. **在 Supabase 项目设置中添加环境变量**：
   - `DASHSCOPE_API_KEY`: 你的阿里云 API Key
   - `SUPABASE_URL`: 项目 URL
   - `SUPABASE_SERVICE_ROLE_KEY`: Service Role Key

2. **部署 Function**：
   ```bash
   supabase functions deploy embed-product
   ```

3. **配置 Webhook**：
   - 进入 Supabase Dashboard → Database → Webhooks
   - 创建新 Webhook，监听 `products` 表的 `INSERT` 和 `UPDATE`
   - 目标 URL: `https://your-project.supabase.co/functions/v1/embed-product`
   - 确保勾选 "Send only changed columns"

---

## 应用层实现 (Nuxt 3)

### 1. 环境变量配置

**`.env` 文件**：
```env
# Supabase
SUPABASE_URL=your_supabase_url
SUPABASE_KEY=your_supabase_anon_key
SUPABASE_SECRET_KEY=your_supabase_service_role_key

# 阿里云百炼
DASHSCOPE_API_KEY=your_dashscope_api_key
```

**`nuxt.config.ts` 配置**：
```typescript
export default defineNuxtConfig({
  runtimeConfig: {
    // 服务端私有配置
    dashscopeApiKey: process.env.DASHSCOPE_API_KEY,
    supabaseService: {
      url: process.env.SUPABASE_URL,
      key: process.env.SUPABASE_SECRET_KEY,
    },
    // 前端公开配置
    public: {
      supabaseUrl: process.env.SUPABASE_URL,
      supabaseKey: process.env.SUPABASE_KEY,
    },
  },
})
```

### 2. 后端 API 实现

**文件路径**：`server/api/ai-chat.ts`

```typescript
import { createOpenAI } from '@ai-sdk/openai'
import { streamText } from 'ai'
import { OpenAI } from 'openai'
import { createClient } from '@supabase/supabase-js'

export default defineEventHandler(async (event) => {
  const config = useRuntimeConfig()
  const { url, key } = config.supabaseService
  const apiKey = config.dashscopeApiKey

  if (!url || !key || !apiKey) {
    throw createError({ statusCode: 500, statusMessage: 'Missing AI Configuration' })
  }

  // 初始化客户端
  const supabaseAdmin = createClient(url, key)
  const aliyun = createOpenAI({
    apiKey: apiKey,
    baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
  })
  const openaiForEmbed = new OpenAI({
    apiKey: apiKey,
    baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
  })

  // 解析用户输入
  const { messages } = await readBody(event)
  const lastMessage = messages[messages.length - 1].content

  try {
    // 1. 生成问题向量
    const embeddingResp = await openaiForEmbed.embeddings.create({
      model: 'text-embedding-v3',
      input: lastMessage.replaceAll('\n', ' '),
      dimensions: 1024,
    })

    // 2. 向量搜索
    const { data: products } = await supabaseAdmin.rpc('match_products', {
      query_embedding: embeddingResp.data[0].embedding,
      match_threshold: 0.5,
      match_count: 5,
    })

    // 3. 构建上下文
    const contextBlock = products?.map((item: any) => {
      const raw = item.product_details
      const name = raw.name?.cn || raw.name?.hk || raw.name?.en || '未命名产品'
      return `[产品]: ${name}\n[AI摘要]: ${item.content}\n---`
    }).join('\n') || '未找到相关产品'

    const systemPrompt = `你是一个服务器硬件专家 (Supercore AI)。
请基于以下产品库回答用户问题。如果库里没有，请说明。
请用繁体中文(香港)或用户使用的语言回答。

【产品库数据】:
${contextBlock}`

    // 4. 流式生成
    const result = await streamText({
      model: aliyun('qwen-plus'),
      messages: [
        { role: 'system', content: systemPrompt },
        ...messages,
      ],
      temperature: 0.3,
    })

    return result.toDataStreamResponse()

  } catch (e: any) {
    console.error('AI Chat Error:', e)
    throw createError({ statusCode: 500, statusMessage: e.message })
  }
})

### 3. 前端组件

**状态管理** (`stores/chat.ts`)
- Pinia Store 处理会话列表同步
- 自动处理会话创建、切换、删除和重命名
- 消息历史记录管理

**前端组件** (`components/ui/AiChat.vue`)
- 集成 `useChatStore` 统一管理状态
- Vercel AI SDK 的 `useChat` hook 处理流式计算
- Swiss Design 风格 UI，支持多会话侧栏
- `useSafeMarkdown` 安全渲染 Markdown 内容
- 自动滚动、加载动画、交互反馈
- **匿名用户管理** (`composables/useAnonymousUser.ts`):
  - 自动生成并持久化唯一标识。
  - 确保即使用户未登录，也能追踪其对话历史和配额消耗。

### 4. AI 管理与成本控制 (Administration)

**服务端限制** (`server/api/ai-chat.ts`):
- **Token 估算**: 根据 Qwen-Plus 模型标准对 Prompt 和响应进行 Token 计算。
- **每日限额**: 每个用户（含匿名）有固定的每日 Token 累计限额，保护后端 API 成本。
- **异常检测**: 异常高频请求自动熔断。

**管理后台统计** (`pages/admin/chat/analytics.vue`):
- **实时看板**: 接入 ECharts 展示每日 Token 消耗趋势、消息数量及响应耗时分布。
- **财务成本**: 自动将 Token 换算为预估美金成本，方便管理。
- **会话溯源**: 支持在后台直接查看任意会话的完整明细。

```vue
<script setup lang="ts">
import { useChat } from 'ai/vue'
import { ref, watch, nextTick } from 'vue'
import { marked } from 'marked'

const isOpen = ref(false)
const scrollContainer = ref<HTMLDivElement | null>(null)

const { messages, input, handleSubmit, isLoading } = useChat({
  api: '/api/ai-chat'
})

marked.setOptions({ breaks: true, gfm: true })

// 自动滚动到底部
watch(messages, async () => {
  await nextTick()
  if (scrollContainer.value) {
    scrollContainer.value.scrollTop = scrollContainer.value.scrollHeight
  }
}, { deep: true })
</script>

<template>
  <div class="fixed bottom-6 right-6 z-50">
    <!-- Chat Window -->
    <div v-if="isOpen" class="w-[350px] h-[500px] bg-white rounded-xl shadow-xl flex flex-col">
      <!-- Header -->
      <div class="p-4 bg-blue-600 text-white flex justify-between">
        <span>Supercore AI</span>
        <button @click="isOpen = false">✕</button>
      </div>

      <!-- Messages -->
      <div ref="scrollContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
        <div v-for="m in messages" :key="m.id"
             :class="['p-3 rounded-lg max-w-[85%]',
                      m.role === 'user' ? 'bg-blue-600 text-white ml-auto' : 'bg-gray-100 mr-auto']">
          <div v-if="m.role === 'assistant'" v-html="marked(m.content)" />
          <div v-else>{{ m.content }}</div>
        </div>
      </div>

      <!-- Input -->
      <form @submit="handleSubmit" class="p-3 border-t flex gap-2">
        <input v-model="input" placeholder="請問有什麼可以幫您？" />
        <button type="submit" :disabled="isLoading">Send</button>
      </form>
    </div>

    <!-- Toggle Button -->
    <button @click="isOpen = !isOpen">💬</button>
  </div>
</template>
```

---

## 踩坑实录

### 坑 1：Nuxt "幽灵报错" (`replace` of undefined)

**现象**：
修改 `.env` 或 `nuxt.config.ts` 后，页面白屏，控制台报错：
```
Cannot read properties of undefined (reading 'replace')
```

**原因**：
Nuxt 的错误页面渲染器 (`youch`) 因缓存问题崩溃了。

**解决**：
```bash
# 1. 停止服务
# 2. 删除缓存
rm -rf .nuxt
rm -rf node_modules/.cache

# 3. 重启
npm run dev
```

### 坑 2：版本依赖地狱

**现象**：
`import { useChat } from 'ai/vue'` 报错找不到模块。

**原因**：
Vercel AI SDK 更新极快，v3 和 v4 API 不兼容。

**解决**：
锁定版本使用 `ai` (3.x)：
```json
{
  "dependencies": {
    "ai": "^3.1.37",
    "@ai-sdk/openai": "^1.0.0",
    "openai": "^4.42.0"
  }
}
```

### 坑 3：Supabase 路由拦截

**现象**：
API 接口在 Postman 能通，但浏览器调用返回 302 或 HTML。

**原因**：
`@nuxtjs/supabase` 模块开启了全局路由保护，未登录用户被拦截。

**解决**：
在 `nuxt.config.ts` 中添加白名单：
```typescript
supabase: {
  redirectOptions: {
    exclude: [
      '/api/ai-chat',
      '/api/products/public',
      // 其他公开 API...
    ]
  }
}
```

### 坑 4：向量维度错误

**现象**：
插入向量时报错 `vector dimension must be 1024`。

**原因**：
阿里云 `text-embedding-v3` 生成的是 1024 维向量，不是 OpenAI 的 1536 维。

**解决**：
确保数据库表和函数都使用 `vector(1024)`。

---

## 部署清单

### 本地开发

1. **安装依赖**：
   ```bash
   npm install
   ```

2. **配置 `.env`**：
   ```env
   SUPABASE_URL=xxx
   SUPABASE_KEY=xxx
   SUPABASE_SECRET_KEY=xxx
   DASHSCOPE_API_KEY=xxx
   ```

3. **启动开发服务器**：
   ```bash
   npm run dev
   ```

### 生产部署

1. **部署 Edge Function**：
   ```bash
   supabase functions deploy embed-product
   ```

2. **配置环境变量**（在部署平台）：
   - `SUPABASE_URL`
   - `SUPABASE_KEY`
   - `SUPABASE_SECRET_KEY`
   - `DASHSCOPE_API_KEY`

3. **构建并部署**：
   ```bash
   npm run build
   # 部署到 Vercel / Netlify 等
   ```

---

## 总结

这套架构非常适合个人开发者或中小项目：

✅ **成本低**：Supabase 免费额度 + 阿里百炼比 OpenAI 便宜 70%+

✅ **自动化**：通过 Edge Function 实现数据同步自动化

✅ **开发体验**：Nuxt 3 全栈开发顺滑，TypeScript 类型安全

⚠️ **注意点**：
- 配置文件改动后记得清缓存
- 版本依赖要锁定，避免自动升级
- API 路由要加入 Supabase 白名单

---

**参考资源**：
- [Vercel AI SDK 文档](https://sdk.vercel.ai/docs)
- [Supabase Edge Functions](https://supabase.com/docs/guides/functions)
- [阿里云百炼文档](https://help.aliyun.com/zh/dashscope/)
- [pgvector 文档](https://github.com/pgvector/pgvector)
