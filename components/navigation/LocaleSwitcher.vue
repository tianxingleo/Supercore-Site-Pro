<template>
  <div class="relative" ref="dropdownRef">
    <button class="flex items-center space-x-1 px-3 py-2 rounded hover:bg-swiss-secondary/10 transition-colors"
      @click="toggleDropdown" aria-label="Switch language">
    ...
    <Transition name="fade">
      <div v-if="dropdownOpen"
        class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-swiss-secondary/10 py-1">
        <button v-for="locale in availableLocales" :key="locale.code"
          class="w-full px-4 py-2 text-left hover:bg-swiss-bg transition-colors flex items-center justify-between"
          :class="{ 'bg-swiss-bg/50': locale.code === currentLocaleCode }" @click="switchLocale(locale.code)">
