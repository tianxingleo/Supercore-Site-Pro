...
<template>
  <div class="space-y-12">
    ...
    <div class="bg-white border border-swiss-text/10 p-4 flex items-center gap-4">
      ...
      <!-- 搜索框 -->
      <input v-model="searchQuery" type="text" placeholder="搜索会话标题或 ID..."
    ...
    <!-- 会话列表 -->
    <div v-if="error" class="bg-red-50 p-4 border border-red-200 text-red-800 text-sm">
      {{ error }}
    </div>

    <div v-else class="bg-white border border-swiss-text/10 overflow-hidden">
      <!-- 加载状态 (Skeleton) -->
      <div v-if="isLoading" class="divide-y divide-swiss-text/5">
        <div v-for="i in 10" :key="i" class="px-6 py-8 flex items-center justify-between">
          <div class="flex-1 space-y-3">
            <div class="h-4 w-1/3 bg-gray-100 relative overflow-hidden">
              <div class="shimmer absolute inset-0"></div>
            </div>
            <div class="h-3 w-1/4 bg-gray-50 relative overflow-hidden">
              <div class="shimmer absolute inset-0"></div>
            </div>
          </div>
          <div class="w-24 h-8 bg-gray-100 relative overflow-hidden">
            <div class="shimmer absolute inset-0"></div>
          </div>
        </div>
      </div>
      ...
      <table v-else class="w-full">
        ...
        <tbody class="divide-y divide-swiss-text/10">
          <tr v-for="session in paginatedSessions" :key="session.id" class="hover:bg-swiss-bg-soft transition-colors">
            <td class="px-6 py-4 text-sm font-mono text-swiss-text-muted">
              #{{ session.id }}
            </td>
            <td class="px-6 py-4">
              <NuxtLink :to="`/admin/chat/sessions/${session.id}`" class="text-sm font-medium hover:underline">
                {{ session.title }}
              </NuxtLink>
            </td>
            ...
            <td class="px-6 py-4">
              <span class="px-2 py-1 text-[9px] font-bold uppercase rounded-sm" :class="{
            ...
            <td class="px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <!-- 查看详情 -->
                <NuxtLink :to="`/admin/chat/sessions/${session.id}`"
                  class="p-1 text-swiss-text-muted hover:text-swiss-text transition-colors" title="查看详情">
                ...
                <!-- 归档 -->
                <button v-if="session.status === 'active'" @click="archiveSession(session.id)"
                  class="p-1 text-swiss-text-muted hover:text-swiss-text transition-colors" title="归档">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <!-- 删除 -->
                <button @click="deleteSession(session.id)" class="p-1 text-red-500 hover:text-red-700 transition-colors"
                  title="删除">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <!-- 空状态 -->
      <div v-if="paginatedSessions.length === 0" class="p-12 text-center text-swiss-text-muted">
